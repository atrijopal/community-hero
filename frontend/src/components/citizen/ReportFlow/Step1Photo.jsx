...
const STRINGS = {
  ...
  subtitle:    'A clear photo helps AI classify the issue accurately',
  takePhoto:   'Take a Photo',
  dragDrop:    'Or drag & drop',
  dropHere:    'Drop here',
  fileLimits:  'JPEG, PNG, WEBP — max 10 MB',
  photoReady:  'Photo ready — AI will analyze this to classify the issue',
  continueBtn: 'Continue →',
  privacy:     'Your photo is processed securely. GPS data is automatically removed.',
...
export default function Step1Photo({ onNext }) {
  ...
  const onDrop = useCallback((accepted) => {
    ...
    const f = accepted[0];
    if (f.size > 10 * 1024 * 1024) { setError(tr.fileTooLarge); return; }
    setFile(f);
    ...
    <div className="space-y-5">
      ...
      {!preview ? (
        <div className="space-y-4">
          ...
          >
            <IconCamera size={20} stroke={2} /> {tr.takePhoto}
          </button>
          <div
            {...getRootProps()}
          ...
          >
            ...
            <p className="font-medium" style={{ color: '#4A4A48' }}>{isDragActive ? tr.dropHere : tr.dragDrop}</p>
            <p className="text-xs mt-1" style={{ color: '#B8B5B0' }}>{tr.fileLimits}</p>
          </div>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="border p-3 flex items-center gap-2 text-sm" style={{ borderColor: '#A7D5B9', backgroundColor: '#E8F5EE', borderRadius: '6px', color: '#1A7A4A' }}>
            <IconSparkles size={14} stroke={1.5} /> {tr.photoReady}
          </div>
          <button
            onClick={() => onNext(file)}
      ...
      <p className="text-xs text-center" style={{ color: '#B8B5B0' }}>{tr.privacy}</p>
    </div>
  );
}
