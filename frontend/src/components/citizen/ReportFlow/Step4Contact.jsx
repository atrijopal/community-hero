...
export default function Step4Contact({ onNext }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold mb-1" style={{ color: '#4A4A48' }}>Contact Details</h2>
        <p className="text-sm" style={{ color: '#7A7875' }}>We'll notify you when your ticket is assigned and resolved</p>
      </div>
      ...
      {user ? (
        <div className="border p-4 flex items-center gap-3" style={{ backgroundColor: '#E8F5EE', borderColor: '#A7D5B9', borderRadius: '8px' }}>
          ...
          <div>
            <p className="font-medium" style={{ color: '#1A7A4A' }}>{user.displayName || 'Signed In'}</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <>
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: '#4A4A48' }}>
                WhatsApp Number <span style={{ color: '#B8B5B0' }}>(optional)</span>
              </label>
              <div className="flex">
              ...
              <p className="text-xs mt-1" style={{ color: '#B8B5B0' }}>We'll send updates via WhatsApp (Twilio sandbox)</p>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1" style={{ color: '#4A4A48' }}>
                Email <span style={{ color: '#B8B5B0' }}>(optional)</span>
              </label>
        ...
        <label className="flex items-center gap-3 cursor-pointer p-3 hover:bg-white transition" style={{ borderRadius: '6px' }}>
          ...
          <span className="text-sm" style={{ color: '#4A4A48' }}>Submit anonymously — don't share any contact info</span>
        </label>
